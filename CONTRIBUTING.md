...
- Migration verification (`.github/workflows/migrations.yml`): `scripts/verify-migrations.mjs` builds a temporary local project from the PR base revision, seeds a fixture through the real RPCs, applies only the PR's new migration files with `supabase migration up --local`, and asserts the fixture's stable facts (ids, versions, amounts, cursor order, credential outcomes, permissions) are unchanged; a fresh job replays HEAD's migrations, regenerates `src/types/database.ts`, and requires an empty diff; a parity job then requires the fresh and upgraded application catalogs (tables, columns, constraints, indexes, function definitions, ACLs, triggers, policies, grants) to be identical. Its Node tests live in `scripts/verify-migrations.test.mjs`.
