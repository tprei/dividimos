...
- Baseline replay on a fresh database, a baseline-freshness check (`./scripts/build-baseline.sh` must produce no diff in `supabase/schema.sql`), declaration parity, and a filename-immutability check, triggered when anything under `supabase/`, `scripts/build-baseline.sh`, `scripts/check-database-invariants.mjs`, `src/types/database.ts`, `vitest.integration.config.mts`, or the workflow changes (`.github/workflows/migrations.yml`).
