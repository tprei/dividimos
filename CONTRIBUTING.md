...
- Android compile on every PR that touches `android/**`, `capacitor.config.ts`, `package.json`, `package-lock.json`, or the workflow: `cap sync android` plus a debug assemble, with no signing secrets. Other PRs skip it, since the app loads the web build from production and the bundled `out/` is a placeholder. The signed release AAB still builds on every push to `main` (`.github/workflows/android.yml`).
