...
- Expense lifecycle is `active` ⇄ `deleted` (soft delete + version history). There is no draft state. Assignment rooms are drafts that live before the ledger in their own tables (`assignment_rooms` and children), never touch `group_balances`, and become an expense only through `finalize_assignment_room`. Optimistic concurrency: mutations send `expected_version_no` and the RPC rejects a mismatch with `stale_version`.
