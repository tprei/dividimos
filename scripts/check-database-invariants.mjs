...
// Only objects this repo's migrations create are audited: the public schema,
// plus realtime policies, which is where the broadcast authorization lives.
// Supabase's own storage, auth and functions schemas are platform-managed and
// ship their own grants.
const AUDITED_POLICY_SCHEMAS = ["public", "realtime"];
...
function auditFailures(state, allowlist) {
  ...
  for (const { table_name, rls_enabled } of state.tables) {
    if (rls_enabled) continue;
    if (allowlist.tablesWithoutRls.includes(`public.${table_name}`)) continue;
    failures.push(`RLS is disabled on public.${table_name}`);
  }
