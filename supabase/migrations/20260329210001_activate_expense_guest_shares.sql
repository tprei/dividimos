-- Update activate_expense to include guest shares in validation.
--
-- The shares_mismatch check now validates:
--   SUM(expense_shares) + SUM(expense_guest_shares) = total_amount
--
-- This is backwards-compatible: expenses without guests have
-- SUM(guest_shares) = 0, so existing behavior is unchanged.
