-- Authorized completion read for finalized assignment rooms.
--
-- Authorization is delegated to the existing `get_assignment_room` read: this
-- RPC calls it with the caller's own arguments and takes the self participant
-- from the returned snapshot, never from an argument. A null member token
-- therefore stays host-only, and the delegated call raises the established
-- stable errors (`unauthenticated`, `not_a_member`, `room_cancelled`,
-- `invalid_token`) without this function re-deciding any of them.
--
-- Identity mapping: finalization stamped every active participant with an
-- immutable `expense_participant_index`, the slot its room-ordered ref
-- occupies in version 1 of the stored expense payload. That stored ref is the
-- stable identity (a user id, or the guest row materialized at finalize
-- time). It is resolved against the CURRENT effective payload, so later
-- edits, removals, reorders and claimed guests report the participant's
-- present index; a missing or dropped slot reports null rather than some
-- other person's position. The read is lock-free and STABLE: it writes
-- nothing, and concurrent claims or edits settle through the mutating RPCs'
-- own rechecks under locks.
--
-- Privacy: only the resolved index and the derived action are returned. User
-- ids, guest ids and other people's membership never leave this function;
-- expense and group ids appear only on the identity-authorized
-- view_expense/accept_invitation actions, so anonymous bearers keep the
-- existing room-view boundary.

CREATE FUNCTION public.get_assignment_room_completion(
  p_room_id uuid,
  p_member_token text
) RETURNS jsonb
