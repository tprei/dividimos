...
--
-- DM-pair validation uses the canonical public.dm_pairs table (immutable,
-- RLS-locked to RPC-only writes) rather than the mutable group_members
-- membership set, so a later member replacing an original DM participant
-- can never gain confirmation authority over that pair's expenses. The
-- normalized chat_expense_confirmation_users link table and full
...
BEGIN
  ...
  -- DM shape: exactly this group's is_dm flag plus its immutable canonical
  -- pair from dm_pairs. Never derived from mutable group_members, which
  -- would let a later-accepted member (after the original participant
  -- left) gain confirmation authority over a DM pair it never belonged to.
  IF NOT v_group.is_dm THEN
    RAISE EXCEPTION USING ERRCODE = 'PST05', MESSAGE = 'permission_denied';
  END IF;

  SELECT dp.user_a, dp.user_b INTO v_dm_user_a, v_dm_user_b
    FROM public.dm_pairs dp
   WHERE dp.group_id = v_group.id;

  IF v_dm_user_a IS NULL THEN
    RAISE EXCEPTION USING ERRCODE = 'PST05', MESSAGE = 'permission_denied';
  END IF;

  v_pair_ids := ARRAY[v_dm_user_a, v_dm_user_b];

  IF NOT (v_caller = ANY (v_pair_ids)) THEN
    RAISE EXCEPTION USING ERRCODE = 'PST05', MESSAGE = 'permission_denied';
  END IF;
