...
-- decline_invitation must honour the same durable exclusion: a user an
-- administrator removed cannot decline the invitation back. The declaration
-- in supabase/schemas/06_rpc_group.sql carries this exact body; the replay
-- chain defines it here so the applied history matches the declaration.
set check_function_bodies = off;
