-- Normalize the balance ledger after a guest claim and after a settlement
-- confirmation (issue #592).
--
-- claim_guest_spot and confirm_settlement are the remaining two balance-
-- writing RPCs. Each appends minimize_group_balances so the stored balances
-- stay canonical at all times, matching activate_expense and record_settlements.
--
-- Both bodies are copied verbatim from their latest effective migrations with
-- one appended PERFORM each. CREATE OR REPLACE preserves the existing ACLs,
-- so no REVOKE/GRANT is re-issued.

-- ============================================================
-- claim_guest_spot(text) — source: 20260813000000_guest_claim_token_rpcs.sql:244
-- Insert minimize after the token-closing step, before the final RETURN. The
-- early already-claimed RETURN performs no balance write and is untouched.
-- ============================================================
...
BEGIN
  ...
  PERFORM public.minimize_group_balances(v_group.id);
...
-- ============================================================
-- confirm_settlement(uuid) — source: 20260802000000_enforce_expense_money_invariants.sql:848
-- Append minimize before the function's closing END. No explicit RETURN
-- (RETURNS void).
-- ============================================================
