-- Issue #477 Slice 5 + #495 enabler: participant_order support in
-- save_expense_draft_graph. This populates expense_allocation_entities
-- (the participant map) and unblocks #495's payer-reachability work.
--
-- Per #477 spec: the nine-argument save_expense_draft_graph must persist
-- the complete participant map. Per #495 spec: the composite FK on
-- expense_payers requires the participant map to exist.
...
DECLARE
  ...
  v_idx                      integer;
  v_kind                     text;
  v_guest_map                jsonb := $ineq${}$ineq$;
  v_share_amount             integer;
  v_payer_amount             integer;
BEGIN
  ...
  LOOP
    ...
    v_guest_map := v_guest_map || pg_catalog.jsonb_build_object(
      v_guest ->> 'local_id', v_guest_id
    );
  ...
  -- Insert participant map from p_participant_order (#477 Slice 5)
  IF pg_catalog.jsonb_typeof(p_participant_order) = 'array'
     AND pg_catalog.jsonb_array_length(p_participant_order) > 0 THEN
    FOR v_idx IN 1..pg_catalog.jsonb_array_length(p_participant_order)
    LOOP
      v_entry := p_participant_order -> (v_idx - 1);
      v_kind := v_entry ->> 'kind';

      IF v_kind = 'user' THEN
        v_user_id := (v_entry ->> 'userId')::uuid;
        SELECT COALESCE(SUM(share_amount_cents), 0) INTO v_share_amount
          FROM public.expense_shares
         WHERE expense_id = v_expense_id AND user_id = v_user_id;
        SELECT COALESCE(SUM(amount_cents), 0) INTO v_payer_amount
          FROM public.expense_payers
         WHERE expense_id = v_expense_id AND user_id = v_user_id;
        INSERT INTO public.expense_allocation_entities (
          expense_id, participant_index, entity_kind, user_id,
          share_amount_cents, payer_amount_cents, net_amount_cents
        ) VALUES (
          v_expense_id, v_idx, 'user', v_user_id,
          v_share_amount, v_payer_amount,
          v_share_amount - v_payer_amount
        );
      ELSIF v_kind = 'guest' THEN
        v_guest_local_id := v_entry ->> 'guestLocalId';
        v_guest_id := (v_guest_map ->> v_guest_local_id)::uuid;
        IF v_guest_id IS NULL THEN
          RAISE EXCEPTION USING ERRCODE = 'PST02', MESSAGE = 'participant_order_guest_not_found';
        END IF;
        SELECT COALESCE(SUM(share_amount_cents), 0) INTO v_share_amount
          FROM public.expense_guest_shares
         WHERE expense_id = v_expense_id AND guest_id = v_guest_id;
        INSERT INTO public.expense_allocation_entities (
          expense_id, participant_index, entity_kind, guest_id,
          share_amount_cents, payer_amount_cents, net_amount_cents
        ) VALUES (
          v_expense_id, v_idx, 'guest', v_guest_id,
          v_share_amount, 0, v_share_amount
        );
      ELSE
        RAISE EXCEPTION USING ERRCODE = 'PST02', MESSAGE = 'invalid_participant_order';
      END IF;
    END LOOP;
  END IF;
...
END;
$$;
-- Re-apply ACLs on the redefined function
