-- Group creators are implicit accepted members. The original draft-save RPC only
-- checked group_members, which rejects a newly created non-DM group before its
-- creator has any explicit membership row.
...
BEGIN
  ...
  IF v_expense_id IS NULL THEN
    -- SECURITY DEFINER bypasses RLS. Match my_accepted_group_ids(): a caller
    -- may create an expense as an accepted member or as the group creator.
    IF NOT EXISTS (
      SELECT 1
        FROM public.groups g
       WHERE g.id = (p_expense->>'group_id')::uuid
         AND (
           g.creator_id = v_caller
           OR EXISTS (
             SELECT 1
