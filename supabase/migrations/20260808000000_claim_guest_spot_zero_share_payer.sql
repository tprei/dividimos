-- Issue #495 Slice 2: claim_guest_spot zero-share-payer amendment.
--
-- Per #495 spec:
--   'an existing user share remains duplicate_participant except when it
--    is exactly zero and the claimant is already an expense_payers user
--    for that same expense. In that one allowed case, lock the zero
--    share and update it to the claimed guest's share amount instead of
--    inserting a duplicate.'
--
-- This preserves the #468-supported path in which a payer with no prior
-- share claims a guest. It is a claim-body amendment, not a guest-payer
-- feature. A nonpayer with a zero share, a payer with a positive share,
-- or a payer from another expense remains ineligible.
