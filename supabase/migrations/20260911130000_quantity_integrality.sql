-- validate_expense_payload checked quantityMilliunits for integrality with a
-- numeric comparison while unitPriceCents and totalPriceCents used a text
-- comparison, so a JSON literal like 1000.0 passed validation and then failed
-- at the bigint cast in the return, leaking "invalid input syntax for type
-- bigint" to the client. All three fields now compare the same way.
