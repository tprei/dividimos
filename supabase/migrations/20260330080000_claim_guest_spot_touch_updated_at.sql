-- Bump expenses.updated_at when a guest claims their spot.
-- This lets useRealtimeExpense fire on the inviter's screen so the
-- unclaimed-guests list updates and the QR modal can auto-close.
