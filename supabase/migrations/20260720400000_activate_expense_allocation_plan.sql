-- ============================================================
-- Issue #468: rewrite public.activate_expense to build the
-- persisted participant map, call build_expense_allocation_plan_edges,
-- validate exact per-entity incidence, persist the plan/edges, and
-- apply user-debtor edges to balances.
--
-- This replaces the per-pair ROUND() + residual-reassignment
-- algorithm (20260516150000 / 20260716000000). For shares [1,1,4] /
-- payers [1,4,1] the old body produced incidence [+1,-5,+4]; the new
-- body produces the exact integer incidence [0,-3,+3] via the
-- deterministic two-cursor matcher in build_expense_allocation_plan_edges.
--
-- The function name and signature are unchanged
-- (public.activate_expense(p_expense_id uuid) RETURNS void). The
-- rename to activate_saved_expense is owned by #467 and lands later.
--
-- Preserve the existing transaction-local DM system_expense insert until #473 lands.
--
-- Forward-only: CREATE OR REPLACE of an existing function. No table
-- or column changes. Existing EXECUTE grants on activate_expense(uuid)
...
COMMENT ON FUNCTION public.activate_expense(uuid) IS
  'Activate a draft expense: build the persisted participant map and the '
  'exact integer allocation plan (#468), validate per-entity incidence, '
  'persist plan/edges, and apply user-debtor edges to balances. '
  'Guest-debtor edges are recorded pending (applied at claim).';

-- CREATE OR REPLACE preserves the existing EXECUTE grant; re-assert it
-- idempotently so the function remains callable by authenticated users.
GRANT EXECUTE ON FUNCTION public.activate_expense(uuid) TO authenticated;

-- The plan-edge -> entity FKs default to NO ACTION, which blocks the
-- expense->entities cascade (allocations still reference the entity rows as
-- the cascade fires). Make them ON DELETE CASCADE so deleting an expense
-- cleanly reaches allocations through both paths (plan and entities).
ALTER TABLE public.expense_balance_allocations
  DROP CONSTRAINT expense_balance_allocations_expense_id_debtor_index_fkey,
  DROP CONSTRAINT expense_balance_allocations_expense_id_creditor_index_fkey;

ALTER TABLE public.expense_balance_allocations
  ADD CONSTRAINT expense_balance_allocations_debtor_entity_fkey
    FOREIGN KEY (expense_id, debtor_index)
    REFERENCES public.expense_allocation_entities(expense_id, participant_index)
    ON DELETE CASCADE,
  ADD CONSTRAINT expense_balance_allocations_creditor_entity_fkey
