...
├── schema.sql              # Snapshot gerado por scripts/build-baseline.sh
└── migrations/             # Baseline aplicado congelado + migrações futuras
...
- `supabase/schemas/` — Declarative SQL: tables, grants, RPCs, realtime, triggers. Source of truth; `supabase/schema.sql` is the current generated snapshot, and `supabase/migrations/` holds the frozen applied baseline plus forward migrations
...
**Schema**: `supabase/schemas/*.sql` is the declarative source of truth. `supabase/schema.sql` is the current snapshot generated by `./scripts/build-baseline.sh`. The applied baseline migration is frozen and must never be regenerated or edited. After editing a schema file, regenerate the snapshot, generate and review a new forward migration with `supabase db diff`, and commit both. `supabase db reset` replays the committed migrations on a fresh local database.
...
./scripts/build-baseline.sh # Regenerate the schema snapshot from supabase/schemas/
supabase db diff -f <migration-name> # Generate a reviewed forward migration while the local stack is stopped
```
...
- `migrations.yml` — replays committed migrations on a fresh database, fails if `supabase/schema.sql` is stale relative to `supabase/schemas/`, checks declaration parity, rejects renamed or deleted migration files, and is triggered when `supabase/schemas/**`, `supabase/schema.sql`, `supabase/migrations/**`, or `scripts/build-baseline.sh` changes.
