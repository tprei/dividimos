...
- `migrations.yml` — replays committed migrations on a fresh database, fails if `supabase/schema.sql` is stale relative to `supabase/schemas/`, checks declaration parity, and rejects renamed or deleted migration files. Triggered when `supabase/schemas/**`, `supabase/schema.sql`, `supabase/migrations/**`, or `scripts/build-baseline.sh` changes.
