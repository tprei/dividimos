...
export function DebtRowButton({ row, onSelect }: DebtRowButtonProps) {
  const firstName = row.counterpartyName.split(" ")[0] ?? row.counterpartyName;
  ...
  return (
    <button
      ...
      aria-label={`${firstName}, ${direction} ${formatBRL(row.amountCents)}, ${group}`}
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex min-w-0 items-center gap-1.5">
          <span className="truncate text-[15px] font-semibold">{firstName}</span>
