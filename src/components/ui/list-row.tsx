...
export function ListRow({ leading, title, subtitle, meta, trailing, className, href, onClick, disabled }: ListRowProps) {
  const classes = cn("flex min-h-11 w-full items-center gap-3 px-3 py-2.5 text-left", (href !== undefined || onClick) && "outline-none transition-colors hover:bg-muted/60 focus-visible:ring-3 focus-visible:ring-inset focus-visible:ring-ring motion-safe:transition-transform motion-safe:active:scale-[0.97] disabled:pointer-events-none disabled:opacity-50", className);
  const content = <>
