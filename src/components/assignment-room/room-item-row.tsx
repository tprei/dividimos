              ...
              <span className="flex shrink-0 flex-wrap items-center justify-end gap-x-2 gap-y-1 max-[380px]:max-w-32">
                {visibleOwners.length > 0 && (
                  <span aria-hidden="true" className="flex shrink-0 items-center py-0.5 pl-0.5">
                    {visibleOwners.map((owner) => <UserAvatar key={owner.id} name={owner.displayName} avatarUrl={owner.avatarUrl} size="xs" className="-ml-0.5 shrink-0 ring-2 ring-card first:ml-0" />)}
                  </span>
                )}
    ...
    <li data-item-id={item.id}>
      ...
      >
        ...
        <span className="flex shrink-0 flex-col items-end gap-1">
          ...
          {availableTicks > 0 && <span className="rounded-full bg-primary/25 px-2 py-0.5 text-[11px] font-semibold text-primary-text">{owners.length === 0 ? "Sem dono" : multiUnit ? `${formatRoomTicks(availableTicks)} sobrando` : "incompleto"}</span>}
        </span>
              ...
              return (
                <li key={claim.participantId} className="flex items-center gap-1">
                  <button type="button" aria-label={`Mudar ${item.description} de ${owner.displayName}`} disabled={disabled || pending} onClick={(event) => onOpen(event.currentTarget, owner.id)} className="flex min-h-14 min-w-0 flex-1 items-center gap-2 rounded-lg px-1 py-2 text-left hover:bg-muted focus-visible:outline-2 focus-visible:outline-primary disabled:opacity-60">
                    <UserAvatar name={owner.displayName} avatarUrl={owner.avatarUrl} size="sm" />
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-1 flex items-center gap-2"><span className="h-1.5 flex-1 overflow-hidden rounded-full bg-muted"><span className="block h-full rounded-full bg-primary" style={{ width }} /></span><span className="text-[10px] text-muted-foreground tabular-nums">{percent}</span></span>
                    </span>
