...
export interface WizardInitInput {
  ...
  me: Me | null;
  step: string;
  onSetPendingGroupId: (groupId: string | null) => void;
...
export function useWizardInit({
  ...
  }, [modes.dm, me, onSetIsDmMode, onSetBillType, onSetStep]);
  ...
  useEffect(() => {
    ...
    onSetStep("info");
  }, [modes.chatDraft, me, onSetIsDmMode, onSetBillType, onSetStep]);
  ...
  }, [modes.editExpenseId, me, onSetIsEditing, onSetBillType, onSetStep]);

  useEffect(() => {
    const groupIdParam = modes.entryGroupId;
    if (!groupIdParam || modes.dm || step === "type" || !me) return;
    if (useBillStore.getState().expense?.groupId) return;

    const snapshot = useAppStore.getState().groups[groupIdParam];
    if (!snapshot) return;

    onSetPendingGroupId(groupIdParam);
    const billStore = useBillStore.getState();
  ...
  }, [step, modes.entryGroupId, modes.dm, me, onSetPendingGroupId]);
