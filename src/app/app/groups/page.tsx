...
export default function GroupsPage() {
  ...
  async function fetchGroups() {
    ...
    for (const m of myMemberships ?? []) {
      ...
      if (m.status === "invited") {
        pendingGroupIds.push(m.group_id);
      }
    }
    ...
    const [{ data: groupData }, { data: allMembers }, { data: activeBillRows }] =
      await Promise.all([
        ...
        nonPendingGroupIds.length > 0
          ? supabase
              .from("bills")
              .select("group_id")
              .in("group_id", nonPendingGroupIds)
              .neq("status", "settled")
    ...
    const pendingInvites: PendingInvite[] = pendingGroupIds.flatMap((gid) => {
      ...
      return [{
        ...
        invitedByName: inviterNameMap.get(inviterId) ?? "",
      }];
    });
