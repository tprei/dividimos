...
describe("parseClaimQrCode", () => {
  it("parses a full HTTPS claim URL", () => {
    const result = parseClaimQrCode(`https://pixwise.app/claim/${UUID}`);
    expect(result).toEqual({ token: UUID, url: `https://pixwise.app/claim/${UUID}` });
  });
  ...
  it("returns null for non-claim URLs", () => {
    ...
    expect(parseClaimQrCode("https://pixwise.app/app")).toBeNull();
    expect(parseClaimQrCode("not a url")).toBeNull();
  });

  it("returns null for invalid UUID format", () => {
    expect(parseClaimQrCode("https://pixwise.app/claim/not-a-uuid")).toBeNull();
    expect(parseClaimQrCode("https://pixwise.app/claim/12345")).toBeNull();
  });

  it("is case-insensitive for hex digits", () => {
    const upper = UUID.toUpperCase();
    const result = parseClaimQrCode(`https://pixwise.app/claim/${upper}`);
