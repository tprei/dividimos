...
describe("evaluateFinancialCompatibility", () => {
  it("is compatible when ready, current schema, and no native version to check", () => {
    expect(evaluateFinancialCompatibility(READY, null)).toEqual({ compatible: true });
  });
  ...
  it("rejects maintenance regardless of schema/native match", () => {
    ...
    expect(evaluateFinancialCompatibility(manifest, 99)).toEqual({
      compatible: false,
  ...
  it("rejects a required schema version above this build's version", () => {
    ...
    expect(evaluateFinancialCompatibility(manifest, null)).toEqual({
      compatible: false,
      issue: { code: "schema_outdated", requiredFinancialSchemaVersion: CURRENT_FINANCIAL_SCHEMA_VERSION + 1 },
    });
  });

  it("ignores native version code on web (null)", () => {
    const manifest: FinancialCompatibilityManifest = { ...READY, minimumNativeVersionCode: 99 };
    expect(evaluateFinancialCompatibility(manifest, null)).toEqual({ compatible: true });
  });
  ...
  it("rejects a native build below the minimum version code", () => {
    ...
    expect(evaluateFinancialCompatibility(manifest, 1)).toEqual({
      compatible: false,
  ...
  it("accepts a native build at or above the minimum version code", () => {
    const manifest: FinancialCompatibilityManifest = { ...READY, minimumNativeVersionCode: 2 };
    expect(evaluateFinancialCompatibility(manifest, 2)).toEqual({ compatible: true });
    expect(evaluateFinancialCompatibility(manifest, 3)).toEqual({ compatible: true });
  });
...
describe("fetchFinancialCompatibility", () => {
  ...
  it("fetches with cache: no-store and returns the evaluated result", async () => {
    ...
    const result = await fetchFinancialCompatibility(null);
