...
describe("fetchSefazReceipt", () => {
  ...
  it("aborts the SEFAZ request when the caller aborts", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(
      (_input, init) =>
        new Promise<Response>((_, reject) => {
          // Platform fetch rejects immediately on an already-aborted signal.
    ...
    const controller = new AbortController();
    const sefazUrl =
      "https://nfce.sefaz.sp.gov.br/consulta?chNFe=12345678901234567890123456789012345678901234";
    const pending = fetchSefazReceipt(sefazUrl, RECEIPT_ACCESS_KEY, controller.signal);
    const assertion = expect(pending).rejects.toBeInstanceOf(SefazFallbackError);
    controller.abort();
    await assertion;
...
describe("application-owned deadlines", () => {
  ...
  it("falls back for SEFAZ when the portal never answers", async () => {
    vi.stubGlobal("fetch", stalledFetch());

    const pending = fetchSefazReceipt(
      "https://nfe.svrs.rs.gov.br/x",
      RECEIPT_ACCESS_KEY,
    );
    const assertion = expect(pending).rejects.toBeInstanceOf(SefazFallbackError);
