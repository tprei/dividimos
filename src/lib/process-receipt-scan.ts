 ...
 */
const SEFAZ_DEADLINE_MS = 15_000;
...
export async function fetchSefazReceipt(
  ...
  try {
    ...
    // A stalled portal offers the same choice as a refusing one: retry, photo,
    // or type it in.
    if (isAbort(error)) {
      throw new SefazFallbackError("A consulta à SEFAZ demorou demais");
    }
    throw error;
  }
