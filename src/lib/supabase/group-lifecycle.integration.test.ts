...
describe.skipIf(!isIntegrationTestReady)("Group lifecycle + RLS", () => {
  ...
  describe("Group SELECT", () => {
    it("creator can read their group", async () => {
      ...
      const { data, error }: { data: Database["public"]["Tables"]["groups"]["Row"] | null } = await aliceClient
        .from("groups")
        .select("*")
        .eq("id", group.id)
        .single();
  ...
  describe("Group members", () => {
    ...
    it("user can accept their own invitation", async () => {
      ...
      const bobClient = authenticateAs(bob);
      const { data, error }: { data: Database["public"]["Tables"]["group_members"]["Row"] | null } = await bobClient
        .from("group_members")
        .update({ status: "accepted" })
        .eq("group_id", group.id)
        .eq("user_id", bob.id)
