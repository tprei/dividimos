...
export async function loadConversationMessages(
  ...
  if (expensesResult.data) {
    for (const row of expensesResult.data) {
      expenses.set(
        row.id,
        expenseRowToExpense(row as Database["public"]["Tables"]["expenses"]["Row"]),
      );
    }
  }
  ...
  if (settlementsResult.data) {
    for (const row of settlementsResult.data) {
      settlements.set(
        row.id,
        settlementRowToSettlement(row as Database["public"]["Tables"]["settlements"]["Row"]),
      );
    }
  }

  // Fetch settlement user profiles not already loaded
