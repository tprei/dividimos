...
function reachesKnownHistory(
  ...
  if (knownOldestMessage !== null && oldest.id === knownOldestMessage) return true;
  return watermark !== null && oldest.id === watermark;
}
...
async function runReconciliation(groupId: string, run: Run): Promise<void> {
  ...
  for (;;) {
    ...
    const page: Conversation = firstPage
    ...
    const messagesDone = page.messagesComplete || page.messageCursor === null;
    const eventsDone = page.eventsComplete || page.eventCursor === null;

    // Both streams must be settled independently before the history is whole.
    if (messagesDone && eventsDone) break;
    if (reachesKnownHistory(page, knownOldestMessage, watermark) && messagesDone) break;

    messageBefore = messagesDone ? null : page.messageCursor;
    eventBefore = eventsDone ? null : page.eventCursor;
    if (messageBefore === null && eventBefore === null) break;
  }
